...
trait Base {
  ...
  def register0(c: Constraint): Unit = {
  ...
  def runN[T](max: Int)(f: Exp[T] => Rel): Seq[String] = {
    ...
    def rec(e: () => Rel)(f: () => Unit): Unit = {
      ...
      try {
        ...
        prog = Nil
        val x = e()
        if (x == No) throw Backtrack
        assert(x == Yes, x)
        val p = prog.reverse
        //println("exec "+p)
        r2(p)
        def r2(xs: List[Rel]): Unit = xs match {
          case Nil => f()
          case Reg(c)::xs =>
            register0(c); r2(xs)
